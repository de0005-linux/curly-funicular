...
برای فعال شدن مسیر سریع سفارشی، فرمان اجرای سرویس باید `python main.py` باشد (در `Procfile` تنظیم شده). پس از deploy، لینک‌های WS را یک بار دوباره import کنید تا `ed=4096` اعمال شود. روی VPS دارای دسترسی root می‌توانید یک بار `sudo bash server-tuning.sh` را اجرا کنید؛ روی Railway نبودن دسترسی sysctl باعث خرابی برنامه نمی‌شود.
