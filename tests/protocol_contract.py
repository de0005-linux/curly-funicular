#!/usr/bin/env python3
"""Contract test for turbo_ws_protocol without installing Uvicorn."""
...
async def main():
    ...
    p.read_paused=False; p.transport=Transport()
    p.logger=types.SimpleNamespace(exception=lambda *a,**k: None)
    marker = b'unique-payload'
    p.frames=[marker]; p.send_receive_event_to_app()
    ...
    while p.queue.qsize() > mod.RX_QUEUE_LOW:
        await p.receive()
    assert p.transport.resumes == 1 and not p.read_paused

    p.writable=asyncio.Event(); p.writable.set(); p.disconnected=False
    p.close_sent=False; p.handshake_complete=True
    p.config=types.SimpleNamespace(ws_per_message_deflate=False)
    p.transport=Transport()
    payload=bytearray(b'x' * 70000)
    await p.turbo_send_bytes(payload)
    ...
    print(f'protocol contract: queue={mod.RX_QUEUE_HIGH}/{mod.RX_QUEUE_LOW}, zero-copy-piece=True, OK')
